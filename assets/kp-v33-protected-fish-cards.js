(function(){
  ...
  function addStyle(){
    ...
    s.textContent=`
      #page-fish .kp-v33-banned-first{order:-999!important;background:rgba(160,48,48,.10)!important;border-color:rgba(160,48,48,.26)!important;color:var(--danger,#a03030)!important;font-weight:900!important;}
      #page-fish .kp-v33-banned-first.active{background:linear-gradient(135deg,var(--danger,#a03030),#b65b3a)!important;color:#fff!important;border-color:transparent!important;}
      ...
      .kp-v33-protected-card{position:relative;overflow:hidden;border:1px solid rgba(160,48,48,.18);background:linear-gradient(180deg,rgba(255,253,248,.98),rgba(247,240,228,.96));border-radius:18px;box-shadow:0 8px 20px rgba(58,39,18,.08);cursor:pointer;min-height:176px;display:flex;flex-direction:column;transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease;text-align:left;padding:0}
  ...
  function ensureBannedFilterFirst(){
    const page=document.getElementById('page-fish');
    if(!page||!window.setFishFilter)return;
    if(page.querySelector('.kp-v33-banned-first')){
      page.querySelector('.kp-v33-banned-first').classList.toggle('active',window.fishFilter==='banned');
      return;
    }
    const firstTab=page.querySelector('.ftab');
    if(!firstTab||!firstTab.parentElement)return;
    const btn=document.createElement('button');
    btn.type='button';
    btn.className='ftab kp-v33-banned-first';
    btn.innerHTML='<i class="ti ti-ban"></i> Tilalomban lévő';
    btn.onclick=function(){window.setFishFilter('banned',btn);};
    firstTab.parentElement.insertBefore(btn,firstTab.parentElement.firstChild);
    btn.classList.toggle('active',window.fishFilter==='banned');
  }
  ...
  function hook(){
    ...
    if(typeof oldSet==='function'&&!oldSet.KP_V33_WRAPPED){
      const wrappedSet=function(f,el){
        ...
        setTimeout(ensureBannedFilterFirst,30);
        return r;
      };
  ...
  setInterval(ensureBannedFilterFirst,1500);
})();

...
// v38.0 · helyreállított sync motor + visszatöltési állapotjelző
(function(){
'use strict';
if(window.KP_MOD_SYNC_V38)return;
window.KP_MOD_SYNC_V38=true;
...
function counts(d){d=d||{};return{sessions:arr(d.sessions).length,locations:arr(d.locations).length,scoutSpots:arr(d.scoutSpots).length,catches:arr(d.sessions).reduce((a,s)=>a+arr(s&&s.catches).length,0),baits:arr(d.baits).length,gear:arr(d.gear).length}}
function meaningful(d){const c=counts(d);return c.sessions||c.locations||c.scoutSpots||c.catches||c.baits||c.gear}
function keyOf(o,p){return String((o&&typeof o==='object'&&(o.id||o.uuid||o.createdAt||(String(o.date||'')+'|'+String(o.location||'')+'|'+String(o.lat||'')+'|'+String(o.lon||''))))||p+'_'+Math.random()).slice(0,240)}
...
async function putJson(c,p,obj,msg,createOnly){const sha=createOnly?null:await getSha(c,p);const body={message:msg||'KapásPont mentés',content:enc(JSON.stringify(obj,null,2)+'\n'),branch:c.branch};if(sha)body.sha=sha;return gh(c,'https://api.github.com/repos/'+encodeURIComponent(c.owner)+'/'+encodeURIComponent(c.repo)+'/contents/'+apiPath(p),{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})}
...
window.githubSyncNow=async function(){if(running){toast('Már fut egy GitHub mentés.');return}running=true;status('GitHub mentés készítése…');try{const c=cfg();check(c);const local=db();if(!meaningful(local))throw new Error('Nincs menthető naplóadat.');const backup={schemaVersion:5,backupType:'github-sync-full',app:'KapásPont',appVersion:'v38.0',exported:new Date().toISOString(),...local};const bp=fpath(c,'backups/full_backup_'+stamp()+'.json');await putJson(c,bp,backup,'KapásPont új backup',true);await putJson(c,fpath(c,'latest/full_backup_latest.json'),backup,'KapásPont latest backup frissítés',false);await putJson(c,fpath(c,'manifest.json'),{schemaVersion:3,app:'KapásPont',appVersion:'v38.0',updated:new Date().toISOString(),backup:bp,latestBackup:fpath(c,'latest/full_backup_latest.json'),latest:fpath(c,'latest/full_backup_latest.json'),counts:counts(backup)},'KapásPont manifest frissítés',false);status('GitHub mentés kész.','ok');toast('GitHub mentés kész.')}catch(e){status('Mentési hiba: '+e.message,'err');toast('Mentési hiba: '+e.message)}finally{running=false}}
window.githubRestoreLatestFromRepo=async function(){try{const c=cfg();check(c);const found=await findLatestBackup(c);if(!found){status('Nem találtam használható mentést.','err');return}const local=db(),lc=counts(local),rc=counts(found.data);if(!confirm('GitHub backup összevonása?\n\nHelyi: '+lc.sessions+' túra, '+lc.scoutSpots+' hely\nGitHub: '+rc.sessions+' túra, '+rc.scoutSpots+' hely\n\nA művelet összevon, nem felülír.'))return;safety();status('Adatok összevonása…');saveDb(mergeDB(local,found.data,found.path));status('Összevonás kész.','ok');toast('GitHub backup összevonva.')}catch(e){status('Visszatöltési hiba: '+e.message,'err');toast('GitHub visszatöltési hiba: '+e.message)}}
window.githubDownloadLatestFromRepo=window.githubRestoreLatestFromRepo;
window.kpRestoreLatestGithubBackup=async function(auto){try{const c=cfg();check(c);const found=await findLatestBackup(c);if(!found)throw new Error('Nem találtam használható GitHub backupot.');if(!auto){const lc=counts(db()),rc=counts(found.data);if(!confirm('Visszatöltsem és összevonjam a legutóbbi GitHub mentést?\n\nHelyi: '+lc.sessions+' túra, '+lc.scoutSpots+' hely\nGitHub: '+rc.sessions+' túra, '+rc.scoutSpots+' hely'))return}safety();saveDb(mergeDB(db(),found.data,found.path));status('GitHub mentés visszatöltve.','ok')}catch(e){if(!auto){status('Visszatöltési hiba: '+e.message,'err');toast('GitHub visszatöltési hiba: '+e.message)}}}

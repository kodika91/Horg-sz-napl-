...
// v39.7 · recursive stripInlinePhotos (minden data: URI mezőből)
(function(){
'use strict';
...
function saveDb(d){const clean=stripInlinePhotos(d);const json=JSON.stringify(clean||{});try{localStorage.setItem(DB_KEY,json)}catch(e){if(String(e).includes('quota')||e.name==='QuotaExceededError'){throw new Error('Tár megtelt, mentés sikertelen. Szabadíts fel helyet!')}else{throw e}}try{if(typeof migrateDB==='function')migrateDB()}catch(e){}refresh();}

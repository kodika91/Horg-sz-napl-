 ...
 * v1.1 · Erősebb túraazonosítás + fogás képének mentése file inputból / előnézetből.
 * Nem nyúl halfaj scrollhoz, csali képmérethez, helykeresőhöz.
 */
(function(){
'use strict';
if(window.KP_SESSION_CATCH_LATE_SAVE_FIX_V11)return;
...
function rememberSessionIdFromClick(el){
  const p=el&&el.closest&&el.closest('[data-session-id]');
  if(p&&p.getAttribute('data-session-id'))lastSelectedSessionId=p.getAttribute('data-session-id');
}
function findSessionIdFromUi(){
  ...
  if(lastSelectedSessionId)return lastSelectedSessionId;
  return '';
}
...
function findSession(d){
  ...
  if(titleTxt){
    const s=ss.find(x=>titleTxt.includes(norm(x.location||x.place||x.name||x.title||''))&&norm(x.location||x.place||x.name||x.title||''));
    if(s)return s;
  }
...
async function fileToDataUrl(file){
  ...
  try{
    ...
    const maxSide=1200;let w=img.naturalWidth,h=img.naturalHeight;const scale=Math.min(1,maxSide/Math.max(w,h));w=Math.round(w*scale);h=Math.round(h*scale);
...
function findCatchPanel(){
  ...
  for(let i=0;n&&n!==document.body&&i<10;i++,n=n.parentElement){
    if(visible(n)&&n.querySelectorAll('input,select,textarea,button').length>=2)return n;
  }
  return document;
}
...
function catchKey(c){return String(c&& (c.id||c.uuid||(String(c.fish||c.hal||'')+'|'+String(c.bait||c.csali||'')+'|'+String(c.weight||c.weightKg||'')+'|'+String(c.length||c.lengthCm||'')+'|'+String(c.createdAt||c.time||''))))}
function addCatchIfMissing(session,c){
  ...
  if(exists){
    const ex=session.catches.find(x=>String(x.fish||x.hal||'')===String(c.fish||c.hal||'')&&String(x.bait||x.csali||'')===String(c.bait||c.csali||'')&&String(x.weight||x.weightKg||'')===String(c.weight||c.weightKg||''));
    if(ex&&c.photo&&!ex.photo){Object.assign(ex,{photo:c.photo,photoData:c.photoData||c.photo,photoName:c.photoName||'',photoStorage:c.photoStorage||'inline-backup'});session.updatedAt=new Date().toISOString();return true;}
    return false;
  }
  session.catches.push(c);
  session.fogasok=session.catches;
  session['fogások']=session.catches;
...
document.addEventListener('click',function(e){rememberSessionIdFromClick(e.target);const b=e.target&&e.target.closest&&e.target.closest('button');if(!b||!catchFormVisible())return;const t=String(b.textContent||'').toLowerCase();if(/fogás|fogas|mentés|mentes|hozzáadás|hozzaadas|save|add/.test(t)){setTimeout(function(){fallbackSaveCatch()},320);}},true);
...
console.log('[late-catch-save] v1.1 aktív · képmentéssel');
})();

// kp-mod-session.js — horgászatok, fogások, fotók
// Tartalom: v30-catch-hotfix · v38-session-photo-to-github
...
(function(){
  ...
  function methods(){const d=getdb();return uniq(['Feeder','Method feeder','Matchbot','Úszós','Rakós','Spicc','Pergetés','Bojlis','Harcsa zás',...(d.sessions||[]).flatMap(s=>[s.method,...(s.catches||[]).map(c=>c.method)])]).sort((a,b)=>a.localeCompare(b,'hu'))}
  ...
  function inEditableField(){const el=document.activeElement;if(!el)return false;const tag=(el.tagName||'').toLowerCase();if(tag==='input'||tag==='select'||tag==='textarea')return true;if(el.isContentEditable)return true;return false;}
  ...
  document.addEventListener('focusout',function(){setTimeout(function(){if(deferredRefresh&&!inEditableField()){deferredRefresh=false;refresh();}},150);},true);
  ...
  function hookImages(){/* v38 kezeli a képnézőt */}

 ...
 * v1.6 · stabilabb Leaflet térkép-felismerés + beépített navigációs panel.
 */
(function(){
'use strict';
if(window.KP_SPOT_NAV_V16)return;
window.KP_SPOT_NAV_V16=true;
...
function discoverMap(){
  ...
  try{
    for(const k of Object.keys(window)){
      const o=window[k];
      if(isMapObject(o)&&o._container&&String(o._container.id||'').includes('spotfinder')){cachedMap=o;window.spotFinderMap=o;return o}
    }
  }catch(e){}
...
function ensureUi(){
  ...
  const wrap=document.createElement('div');wrap.id='sf-nav-overlay';wrap.style.cssText='position:fixed;left:10px;right:10px;top:72px;z-index:99999;display:none';
  ...
  document.body.appendChild(wrap);qs('#sf-nav-follow').onclick=function(){follow=true;update();toast('📍 Visszatalálás követés aktív.')};qs('#sf-nav-stop').onclick=stop;
}
...
const oldList=window.renderSpotFinderList;if(typeof oldList==='function'&&!oldList.KP_SPOT_NAV_V16_WRAPPED){window.renderSpotFinderList=function(){const r=oldList.apply(this,arguments);setTimeout(attachButtons,160);setTimeout(attachButtons,900);return r};window.renderSpotFinderList.KP_SPOT_NAV_V16_WRAPPED=true}
setInterval(update,3000);setInterval(attachButtons,1500);setTimeout(attachButtons,500);
console.log('[spot-nav] v1.6 aktív');
})();
...
function ensureUi(){
  ...
  wrap.innerHTML='<div style="background:rgba(255,255,255,.96);backdrop-filter:blur(10px);border-radius:16px;padding:12px 14px;box-shadow:0 6px 18px rgba(0,0,0,.18)"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div><div id="sf-nav-name" style="font-weight:800;font-size:15px;color:#2a2018">Célpont</div><div id="sf-nav-dist" style="font-size:13px;color:#6b5840">GPS keresése…</div></div><div style="display:flex;gap:8px"><button id="sf-nav-follow" style="border:0;border-radius:10px;padding:10px 12px;font-weight:800;background:#2c6e7a;color:#fff">Kövess</button><button id="sf-nav-stop" style="border:0;border-radius:10px;padding:10px 12px;font-weight:800;background:#efe7dc;color:#2a2018">Leállítás</button></div></div></div>';
...
function startGps(){
  ...
  gpsWatch=navigator.geolocation.watchPosition(function(p){
    ...
    update();
  },function(err){toast('GPS hiba: '+err.message)},{enableHighAccuracy:true,maximumAge:2000,timeout:15000});
}
...
function setTarget(s){
  ...
  if(!Number.isFinite(target.lat)||!Number.isFinite(target.lon)){toast('A hely GPS koordinátája hiányzik.');return}
...
function update(){
  ...
  const label='Távolság: '+fmt(di)+(me.acc?' · ±'+Math.round(me.acc)+' m':'')+(di<15?' · Megérkeztél ✓':'');
...
function stop(){target=null;if(line){try{line.remove()}catch(e){} line=null;}if(marker){try{marker.remove()}catch(e){} marker=null;}const ov=qs('#sf-nav-overlay');if(ov)ov.style.display='none';stopGps();}
...
function attachButtons(){
  ...
  qsa('.card,.spot-card,.list-card,.item-list-card').forEach(function(row){

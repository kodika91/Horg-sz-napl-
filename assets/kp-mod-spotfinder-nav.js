 ...
 * v1.7 · navigációs sáv térképhez igazítva.
 */
(function(){
'use strict';
if(window.KP_SPOT_NAV_V17)return;
window.KP_SPOT_NAV_V17=true;
...
function discoverMap(){
  ...
  const candidates=[];try{candidates.push(window.spotFinderMap,window.map,window.leafletMap)}catch(e){}
...
function positionOverlay(){
  ...
  if(host&&host!==document.body){
    ...
    ov.style.left=Math.max(8,r.left+pad)+'px';
    ov.style.width=Math.max(260,r.width-pad*2)+'px';
    ov.style.right='auto';
    ov.style.top=Math.max(8,r.top+pad)+'px';
    ov.style.maxWidth='calc(100vw - 16px)';
  }else{
    ov.style.position='fixed';ov.style.left='10px';ov.style.right='10px';ov.style.width='auto';ov.style.top='72px';
  }
}
...
function ensureUi(){
  ...
  wrap.innerHTML='<div style="background:rgba(255,255,255,.98);backdrop-filter:blur(10px);border-radius:16px;padding:12px 14px;box-shadow:0 6px 18px rgba(0,0,0,.22);border:2px solid rgba(44,110,122,.28);box-sizing:border-box"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div><div id="sf-nav-name" style="font-weight:800;font-size:15px;color:#2a2018">🎯 Visszatalálás aktív</div><div id="sf-nav-dist" style="font-size:13px;color:#6b5840">📡 Indítás…</div></div><div style="display:flex;gap:8px"><button id="sf-nav-follow" style="border:0;border-radius:10px;padding:10px 12px;font-weight:800;background:#2c6e7a;color:#fff">Kövess</button><button id="sf-nav-stop" style="border:0;border-radius:10px;padding:10px 12px;font-weight:800;background:#efe7dc;color:#2a2018">Leállítás</button></div></div></div>';
  document.body.appendChild(wrap);qs('#sf-nav-follow').onclick=function(){follow=true;update();toast('📍 Visszatalálás követés aktív.')};qs('#sf-nav-stop').onclick=stop;positionOverlay();
}
...
function setTarget(s,btn){
  ...
  if(!window.L){showStatus(s.name,'⚠️ A térkép könyvtár még nem töltött be.');toast('⚠️ A térkép még nem töltött be.');return}
  let m=map();
  if(!m){showStatus(s.name,'⚠️ Térkép inicializálása… nyomd meg újra 1 mp múlva.');setTimeout(()=>{if(target||s)setTarget(s,btn)},900);return}

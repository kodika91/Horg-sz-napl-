 ...
 * v1.3 · aktív állapotjelző és jobb visszajelzés.
 */
(function(){
'use strict';
if(window.KP_SPOT_NAV_V13)return;
window.KP_SPOT_NAV_V13=true;

let target=null,line=null,marker=null,follow=true,gpsWatch=null,lastPos=null;
function qs(s,r=document){return r.querySelector(s)}
function qsa(s,r=document){return Array.from(r.querySelectorAll(s))}
function toast(m){try{typeof showToast==='function'?showToast(m):alert(m)}catch(e){}}
...
function ensureUi(){
  ...
  wrap.innerHTML='<div style="background:rgba(255,255,255,.97);backdrop-filter:blur(10px);border-radius:16px;padding:12px 14px;box-shadow:0 6px 18px rgba(0,0,0,.18);border:2px solid rgba(44,110,122,.22)"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div><div id="sf-nav-name" style="font-weight:800;font-size:15px;color:#2a2018">🎯 Célpont</div><div id="sf-nav-dist" style="font-size:13px;color:#6b5840">📡 GPS keresése…</div></div><div style="display:flex;gap:8px"><button id="sf-nav-follow" style="border:0;border-radius:10px;padding:10px 12px;font-weight:800;background:#2c6e7a;color:#fff">Kövess</button><button id="sf-nav-stop" style="border:0;border-radius:10px;padding:10px 12px;font-weight:800;background:#efe7dc;color:#2a2018">Leállítás</button></div></div></div>';
...
function startGps(){
  ...
  const distEl=qs('#sf-nav-dist');if(distEl)distEl.textContent='📡 GPS pozíció keresése…';
  gpsWatch=navigator.geolocation.watchPosition(function(p){
    lastPos={lat:p.coords.latitude,lon:p.coords.longitude,acc:p.coords.accuracy||0};
    window._gpsLat=lastPos.lat;window._gpsLon=lastPos.lon;
    update();
  },function(err){toast('GPS hiba: '+err.message);const d=qs('#sf-nav-dist');if(d)d.textContent='⚠️ GPS hiba';},{enableHighAccuracy:true,maximumAge:2000,timeout:15000});
}
function stopGps(){if(gpsWatch){try{navigator.geolocation.clearWatch(gpsWatch)}catch(e){} gpsWatch=null;}}
function setTarget(s){
  if(!s||!window.L||!map())return;
  ensureUi();
  target={id:s.id,name:s.name||'Horgászhely',lat:Number(s.lat),lon:Number(s.lon)};
  if(!Number.isFinite(target.lat)||!Number.isFinite(target.lon)){toast('⚠️ A hely GPS koordinátája hiányzik.');return}
...
function update(){
  ...
  const me=current();
  if(!me){const d=qs('#sf-nav-dist');if(d)d.textContent='📡 GPS pozíció keresése…';return;}
  const di=dist(me,target);
  const label='📏 '+fmt(di)+(me.acc?' · ±'+Math.round(me.acc)+' m':'')+(di<15?' · ✅ Megérkeztél':'');
  const el=qs('#sf-nav-dist');if(el)el.textContent=label;
  if(line){try{line.remove()}catch(e){}}
  line=L.polyline([[me.lat,me.lon],[target.lat,target.lon]],{weight:4,opacity:.82}).addTo(map());
  if(follow){const b=L.latLngBounds([[me.lat,me.lon],[target.lat,target.lon]]);map().fitBounds(b,{padding:[60,60],maxZoom:17});}
}
function stop(){target=null;if(line){try{line.remove()}catch(e){} line=null;}if(marker){try{marker.remove()}catch(e){} marker=null;}const ov=qs('#sf-nav-overlay');if(ov)ov.style.display='none';stopGps();toast('🛑 Visszatalálás leállítva.')}
...
function attachButtons(){const db=(typeof getDB==='function'?getDB():{});const spots=(db.scoutSpots||[]);qsa('.card,.spot-card,.list-card,.item-list-card').forEach(function(row){const txt=(row.textContent||'').trim();const spot=spots.find(s=>s&&s.name&&txt.includes(String(s.name).trim()));if(!spot)return;cleanupDupes(row);if(row.querySelector('.kp-nav-btn'))return;const nav=document.createElement('button');nav.type='button';nav.className='kp-nav-btn';nav.textContent='📍 Visszatalálás';nav.style.cssText='margin-top:8px;width:100%;border:0;border-radius:12px;padding:11px 12px;font-weight:800;background:linear-gradient(135deg,#2c6e7a,#3a8a99);color:#fff';nav.onclick=function(){setTarget(spot)};const actions=row.querySelector('.spot-actions,.actions,.btn-row');if(actions&&actions.parentNode===row)actions.insertAdjacentElement('afterend',nav); else row.appendChild(nav);cleanupDupes(row)})}
const oldList=window.renderSpotFinderList;
if(typeof oldList==='function'&&!oldList.KP_SPOT_NAV_V13_WRAPPED){window.renderSpotFinderList=function(){const r=oldList.apply(this,arguments);setTimeout(attachButtons,160);setTimeout(attachButtons,900);return r};window.renderSpotFinderList.KP_SPOT_NAV_V13_WRAPPED=true}
setInterval(update,3000);
setInterval(attachButtons,1500);
setTimeout(attachButtons,500);
console.log('[spot-nav] v1.3 aktív');
})();
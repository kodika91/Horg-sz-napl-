...
/* ====== EGYETLEN, ÚR NAGYÍTÓ (a v30/v37 nagyítóit kizárjuk) ==============
   A diagnózis szerint három script verekszik ugyanazon a #kp-img-view-n.
   Megoldás: a v38 SAJÁT nagyítót nyit, és a kattintást ELKAPJA capture
   fázisban stopImmediatePropagation-nel, így a v30/v37 onclick/listener
   NEM fut le -> nem írja vissza thumbnailre a képet. */
function ownViewer(){
  let v=document.getElementById('kp-v38-view');
  if(v)return v;
  v=document.createElement('div');
  v.id='kp-v38-view';
  v.style.cssText='position:fixed;inset:0;z-index:100000;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,.88);padding:10px';
  v.innerHTML='<div style="position:relative;width:96vw;height:88dvh;background:#101816;border-radius:20px;overflow:hidden;display:flex;align-items:center;justify-content:center"><button id="kp-v38-x" style="position:absolute;right:10px;top:10px;border:0;border-radius:50%;width:40px;height:40px;background:rgba(0,0,0,.5);color:#fff;font-size:26px;z-index:2">×</button><div id="kp-v38-spin" style="position:absolute;color:#cfe;font-size:13px;opacity:.85">Éles kép betöltése…</div><img alt="Fogás fotó" style="width:100%;height:100%;object-fit:contain;display:block"></div>';
  document.body.appendChild(v);
  v.addEventListener('click',function(e){ if(e.target===v||e.target.id==='kp-v38-x') v.style.display='none'; });
  return v;
}
function openOwnViewer(img){
  const v=ownViewer();
  const out=v.querySelector('img');
  const spin=v.querySelector('#kp-v38-spin');
  const src=img.getAttribute('src')||'';
  out.src=src;                 // előbb a meglévő (thumbnail vagy már éles) kép
  v.style.display='flex';
  const path=img.dataset.full||pathForThumbExact(src)||((img.dataset.full=='')?null:null);
  if(path){
    if(spin)spin.style.display='block';
    loadFull(path).then(u=>{ if(u){out.src=u;} if(spin)spin.style.display='none'; }).catch(()=>{ if(spin)spin.style.display='none'; });
  } else {
    if(spin)spin.style.display='none';
  }
}

/* Kattintás elkapása CAPTURE fázisban: ha fogásképre kattintottak, mi kezeljük,
   és megállítjuk a többi kezelőt (v30/v37), hogy ne nyissák meg a saját,
   thumbnailes nagyítójukat. */
document.addEventListener('click',function(e){
  const img=e.target&&e.target.closest?e.target.closest('#session-detail-wrap img,.catch-photo-preview'):null;
  if(img){
    // csak akkor vesszük át, ha van hozzá éles GitHub-út (különben hagyjuk a régit)
  ...
  setTimeout(upgradeImages,150);
},true);

setInterval(upgradeImages,1500);

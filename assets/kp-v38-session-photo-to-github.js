...
const MAX_SIDE=1600,MAX_MB=1.4,THUMB_SIDE=360;
...
async function compress(file,maxSide,maxMb,q0){const ii=await imgInfo(file);let w=ii.w,h=ii.h;const sc=Math.min(1,maxSide/Math.max(w,h));w=Math.max(1,Math.round(w*sc));h=Math.max(1,Math.round(h*sc));const cv=document.createElement('canvas');cv.width=w;cv.height=h;const x=cv.getContext('2d');x.fillStyle='#fff';x.fillRect(0,0,w,h);x.drawImage(ii.img,0,0,w,h);let q=q0,b=await canvasBlob(cv,q);while(maxMb&&b.size>maxMb*1024*1024&&q>.42){q-=.08;b=await canvasBlob(cv,q)}return b}
...
async function queue(file){
  ...
  pending.push(item);
  try{
    const full=await compress(file,MAX_SIDE,MAX_MB,.82);
...
function upgradeImages(){
  const imgs=document.querySelectorAll('#session-detail-wrap img, .catch-photo-preview, #kp-img-view img');

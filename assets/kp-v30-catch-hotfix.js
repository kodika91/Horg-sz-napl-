(function(){
  ...
  function methods(){const d=getdb();return uniq(['Feeder','Method feeder','Matchbot','Match','Úszós','Rakós','Spicc','Pergetés','Bojlis','Harcsázás',...(d.sessions||[]).flatMap(s=>[s.method,...(s.catches||[]).map(c=>c.method)])]).sort((a,b)=>a.localeCompare(b,'hu'))}
  function list(id,arr){let el=document.getElementById(id);if(!el){el=document.createElement('datalist');el.id=id;document.body.appendChild(el)}el.innerHTML=arr.map(v=>'<option value="'+esc(v)+'"></option>').join('')}
  function refresh(){list('bait-options',baits());list('method-options',methods());document.querySelectorAll('#ac-bait,input[list="bait-options"]').forEach(i=>i.setAttribute('list','bait-options'));document.querySelectorAll('#ac-method,input[placeholder*="Feeder"],input[placeholder*="Match"],input[placeholder*="módszer" i]').forEach(i=>i.setAttribute('list','method-options'))}
  ...
  async function loadGithubImage(path){
    const c=cfg(); if(!path||!c.owner||!c.repo||!c.branch||!c.token)return null;
    const api=String(path).split('/').map(encodeURIComponent).join('/');
    const r=await fetch('https://api.github.com/repos/'+encodeURIComponent(c.owner)+'/'+encodeURIComponent(c.repo)+'/contents/'+api+'?ref='+encodeURIComponent(c.branch),{headers:{Accept:'application/vnd.github+json',Authorization:'Bearer '+c.token,'X-GitHub-Api-Version':'2022-11-28'}});
    if(!r.ok)return null;
    const d=await r.json(); const raw=String(d.content||'').replace(/\n/g,''); if(!raw)return null;
  ...
  function findOriginalForImg(img){
    const src=img&&img.src||''; const d=getdb();
    for(const s of (d.sessions||[])){
      for(const c of (s.catches||[])){
        if(!c)continue;
        const small=String(c.photo||'');
        if(src===small || src.endsWith(small.slice(-60)) || (small&&src.startsWith('data:image')&&small.startsWith('data:image')&&src.slice(0,80)===small.slice(0,80))){
          return c.photoPath || (c.photoRef&&c.photoRef.path) || (c.photoRef&&c.photoRef.relativePath) || small;
        }
      }
    }
    return src;
  ...
  async function openImgFromElement(img){
    if(!img)return;const v=viewer();const out=v.querySelector('img');out.src=img.src;v.classList.add('show');
    const orig=findOriginalForImg(img);
    if(orig && !String(orig).startsWith('data:image') && orig!==img.src){try{const blob=await loadGithubImage(orig);if(blob)out.src=blob}catch(e){console.warn('Nagy kép nem tölthető:',e)}}
  }

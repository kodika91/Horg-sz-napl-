(function(){
  ...
  function patchToast(){if(window.__kpV32ToastPatched)return;const old=window.showToast;if(typeof old!=='function')return;window.showToast=function(msg){const t=String(msg||'');if(t.toLowerCase().includes('mentés kész')||t.toLowerCase().includes('mentési hiba'))setTimeout(hideSaving,550);return old.apply(this,arguments)};window.__kpV32ToastPatched=true;}
  ...
  const oldShow=window.showPage;if(typeof oldShow==='function'){window.showPage=function(){const r=oldShow.apply(this,arguments);setTimeout(()=>{updateBack();patchToast();patchSync();},80);return r;};}
  function boot(){css();backBtn();updateBack();patchToast();patchSync();}
  boot();setInterval(()=>{updateBack();patchToast();patchSync();if(!saving){const o=document.getElementById('kp-save-overlay');if(o)o.classList.remove('show')}},900);
})();
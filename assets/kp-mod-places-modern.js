...
function places(){
  ...
  out.forEach(function(p,i){
    ...
    p._idx=i;
    p._dist=distanceKm(num(window.lat),num(window.lon),num(p.lat),num(p.lon));
...
function render(){
  ...
  var sel=list.find(function(p){return p._key===state.selected})||list[0];
  if(sel)state.selected=sel._key;
  if(map){try{map.remove()}catch(e){}map=null;markers=[]}
  page.innerHTML='<div class="kpl">'+leftHtml(list.length)+'<main><h1>Helyek</h1><p class="kpl-sub">Kezeld és fedezd fel horgászhelyeidet a térképen.</p>'+statHtml(list)+mapHtml()+'<div class="kpl-tabs"><button class="kpl-tab active">Összes hely ('+list.length+')</button><button class="kpl-tab">Kedvencek</button><button class="kpl-tab">Legtöbbször használt</button><button class="kpl-tab">Közelemben</button></div><section class="kpl-place-list">'+(list.map(listCard).join('')||'<div class="kpl-panel">Még nincs mentett hely.</div>')+'</section></main>'+detail(sel,list.indexOf(sel))+'</div>';
  wire(page);
  setTimeout(function(){initMap(list,sel)},80);
}

function initMap(list,sel){
  var el=document.getElementById('kpl-map');
  if(!el||typeof L==='undefined')return;
  var start=sel&&sel.lat&&sel.lon?[num(sel.lat),num(sel.lon)]:[47.95,21.38];
  map=L.map(el,{zoomControl:false}).setView(start,sel?11:8);
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:18,attribution:''}).addTo(map);
  list.forEach(function(p){
    if(!p.lat||!p.lon)return;
    var icon=L.divIcon({className:'',html:'<div class="kpl-marker '+(p.favorite?'fav':'')+'"><span>'+(p.favorite?'★':'⌖')+'</span></div>',iconSize:[30,30],iconAnchor:[15,28]});
    var m=L.marker([num(p.lat),num(p.lon)],{icon:icon}).addTo(map).bindPopup('<b>'+esc(p.name||'Hely')+'</b><br>'+esc(typeOf(p)));
    m.on('click',function(){state.selected=p._key;lastSignature='';render()});
    markers.push(m);
  });
  if(markers.length){try{map.fitBounds(L.featureGroup(markers).getBounds().pad(.18))}catch(e){}}
}

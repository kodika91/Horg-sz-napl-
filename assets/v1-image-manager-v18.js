/* V18 · Képkezelő: halfaj/felszerelés képek külön GitHub mappába + image-index.json */
(function(){
  const CFG={
  ...
  async function loadIndex(){
    ...
    imageIndex=Object.assign({version:1,updatedAt:null,fish:{},gear:{},bait:{},places:{}},imageIndex||{});
    return imageIndex;
  }
  ...
  function collectTargets(cat){
    ...
    ['GEAR_DB','gearDB','GEAR','BaitsDB','BAIT_DB'].forEach(k=>{ if(Array.isArray(window[k])) window[k].forEach(x=>arr.push({id:slug(x.id||x.name||x.title),name:x.name||x.title||x.id})); });
  ...
  function preview(){
    ...
    const target=qs('#v18-im-target'); const custom=qs('#v18-im-custom');
    const id=target?.value==='__custom'?slug(custom?.value):target?.value;
    name.textContent=id||'Nincs kiválasztott cél';
    if(selectedImage){box.innerHTML=`<img src="${selectedImage.url}" alt="előnézet">`; meta.textContent=`${selectedImage.w}×${selectedImage.h}px · ${(selectedFile.size/1024/1024).toFixed(2)} MB`;}
    else{box.innerHTML='<i class="ti ti-photo"></i>'; meta.textContent='Minimum 800×500 px, max. 5 MB';}
  }
  ...
  function injectPanel(){
    ...
    wrap.innerHTML=`
      <div class="v18-im-head"><div><div class="v18-im-title">Képkezelő</div><div class="v18-im-sub">Halfajokhoz, felszerelésekhez és később csalikhoz/helyekhez tudsz képet feltölteni külön GitHub mappába. Nem az indexbe kerül.</div></div></div>
      <div class="v18-im-grid">
        ...
        <div class="v18-im-field" id="v18-im-custom-wrap" style="display:none"><label>Egyedi azonosító</label><input id="v18-im-custom" placeholder="pl. szilvaorru_keszeg"></div>
        ...
        <div class="v18-im-file"><input id="v18-im-file" type="file" accept="image/jpeg,image/png,image/webp"><div class="v18-im-sub">Követelmény: minimum ${CFG.minW}×${CFG.minH}px, maximum ${CFG.maxMB} MB. Mentéskor: assets/fish, assets/gear, assets/bait vagy assets/places.</div></div>
  ...
  async function saveImage(){
    try{
      ...
      const cat=qs('#v18-im-cat').value; const target=qs('#v18-im-target').value; const custom=qs('#v18-im-custom').value;
      const id=target==='__custom'?slug(custom):target; if(!id)throw new Error('Válassz cél elemet vagy adj meg egyedi azonosítót.');
      const filename=id+'.'+ext(selectedFile.name,selectedFile.type); const path=(CFG.cats[cat]||'assets/images')+'/'+filename;
      ...
      let idx=await loadIndex(); idx=Object.assign({version:1,fish:{},gear:{},bait:{},places:{}},idx||{}); idx[cat]=idx[cat]||{}; idx[cat][id]=path; idx.updatedAt=new Date().toISOString();
  ...
  function setCardImage(card,src){
    const wrap=card.querySelector('.fish-img-wrap,.gear-img-wrap,.item-img-wrap,.card-img-wrap');

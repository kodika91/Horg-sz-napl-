 ...
 * v1.2 · Egyetlen GitHub beállítási panel, ismétlődő renderelés nélkül.
 */
(function(){
'use strict';
if(window.KP_SETTINGS_SIMPLE_V12)return;
window.KP_SETTINGS_SIMPLE_V12=true;
...
function isSettingsLike(){
  ...
  const t=titleText();
  return !!qs('#page-settings.active,.page.active[id*="settings"],[data-page="settings"].active')||id.includes('export')||id.includes('setting')||t.includes('beállítás')||t.includes('export')||t.includes('adatmentés');
}
function scope(){return qs('#page-settings')||qs('#page-export')||qs('.page.active')||document.body}
...
function panel(){
  ...
  wrap.innerHTML=`
    ...
    <p>GitHub kapcsolat, mentés és a legutóbbi mentés összevonó betöltése. A betöltés nem írja felül a meglévő bejegyzéseket.</p>
    ...
    <p class="kp-ss-note">A képkezelő GitHub fájlokba menti a halfaj, csali és felszerelés képeit.</p>`;
...
function hideNoise(sc){
  const bad=['túraösszesítő export','aktuális túra html','pdf / nyomtatás','importálás','json betöltése','adatok törlése','összes törlése','csv letöltése','exportálás','github szinkron','github mentés és visszatöltés','legutóbbi github mentés'];
  qsa('.card,section,article',sc).forEach(el=>{
    if(el.id==='kp-simple-settings-panel'||el.id==='v18-image-manager'||el.closest('#kp-simple-settings-panel'))return;
...
function simplify(){
  ...
  const sc=scope();
  qsa('#kp-simple-settings-panel').slice(1).forEach(x=>x.remove());
...
if(typeof oldShow==='function'&&!oldShow.KP_SETTINGS_SIMPLE_V12_WRAPPED){window.showPage=function(){const r=oldShow.apply(this,arguments);setTimeout(simplify,120);setTimeout(simplify,700);return r};window.showPage.KP_SETTINGS_SIMPLE_V12_WRAPPED=true}
setTimeout(simplify,700);setTimeout(simplify,1800);
console.log('[settings-simple] v1.2 aktív');
})();
(function(){
  ...
  async function put(c,p,b64,msg){
    ...
    for(let attempt=1;attempt<=3;attempt++){
      let sha=null;
      try{const old=await gh(c,url+'?ref='+encodeURIComponent(c.branch));sha=old&&old.sha}catch(e){if(!String(e.message||'').includes('Not Found')){lastErr=e;}}
      ...
      catch(e){lastErr=e;const m=String(e.message||'');if(m.includes('does not match')||m.includes('is at')||e.status===409){log('GitHub SHA ütközés, újrapróbálás: '+p+' ('+attempt+'/3)');await new Promise(r=>setTimeout(r,450*attempt));continue}throw e}
    }
  ...
  async function sync(auto){
    ...
    try{
      ...
      const backup={schemaVersion:4,backupType:'github-sync-full',app:'KapásPont',appVersion:'v29.1',release:'stable-sync-sha-retry',exported:new Date().toISOString(),storage:storage(pack.db,pack.imgs.length),fishImageAssets:[],...pack.db};
      const js=JSON.stringify(backup,null,2);if(js.length<100)throw new Error('Túl kicsi JSON mentés, nem írom felül a backupot.');
      const ts=stamp();
      await put(c,fpath(c,'backups/full_backup_'+ts+'.json'),enc(js),'KapásPont időbélyeges backup');
      ...
      toast('Mentés kész: '+new Date().toLocaleString('hu-HU'));log('Mentés kész. SHA ütközés elleni védelemmel.');if(typeof renderStorageOverview==='function')renderStorageOverview();
    }catch(e){console.error(e);toast('Mentési hiba: '+e.message);log('Mentési hiba: '+e.message)}
    finally{running=false;}
  }
  function install(){if(typeof getDB!=='function'){setTimeout(install,250);return}window.githubSyncNow=function(auto){return sync(!!auto)};log('Stabil GitHub mentés aktív, SHA retry védelemmel.');}
  install();
})();
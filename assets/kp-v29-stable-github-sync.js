(function(){
  ...
  async function put(c,p,b64,msg,mode){
    ...
    if(mode==='create')return gh(c,url,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg||'KapásPont új fájl',content:b64,branch:c.branch})});
    let lastErr=null;
    for(let attempt=1;attempt<=4;attempt++){
      let sha=null;try{const old=await gh(c,url+'?ref='+encodeURIComponent(c.branch)+'&t='+Date.now());sha=old&&old.sha}catch(e){if(!String(e.message||'').includes('Not Found'))lastErr=e}
  ...
  async function sanitize(db,c){const out=JSON.parse(JSON.stringify(db||{}));const imgs=[];async function add(dataUrl,rel,label){if(!String(dataUrl||'').startsWith('data:image/'))return null;const p=fpath(c,rel+'.'+ext(dataUrl));imgs.push({path:p,base64:ib64(dataUrl),label});return {storage:'github',path:p,relativePath:rel+'.'+ext(dataUrl),thumb:await thumb(dataUrl),bytes:Math.round(ib64(dataUrl).length*0.75),createdAt:new Date().toISOString()}}
    for(const s of (out.sessions||[])){const base='images/sessions/'+(s.date||new Date().toISOString().slice(0,10))+'/'+slug(s.id||s.location||Date.now());let i=0;for(const ca of (s.catches||[])){i++;if(ca.photo&&String(ca.photo).startsWith('data:image/')){const r=await add(ca.photo,base+'_catch_'+pad(i),'fogás kép');if(r){ca.photo=r.thumb;ca.photoRef=r;ca.photoPath=r.path;ca.photoStorage='github'}}}let j=0;for(const ev of (s.events||[])){j++;if(ev.photo&&String(ev.photo).startsWith('data:image/')){const r=await add(ev.photo,base+'_event_'+pad(j),'esemény kép');if(r){ev.photo=r.thumb;ev.photoRef=r;ev.photoPath=r.path;ev.photoStorage='github'}}}}
  ...
  async function guard(c,local){
    const lc=countData(local||{});
    if(!hasMeaningful(local))throw new Error('Biztonsági védelem: nincs helyi túra/hely/fogás adat, ezért nem írom felül a GitHub mentést. Előbb töltsd vissza a backupot.');
    const remote=await getJson(c,fpath(c,'latest/full_backup_latest.json'));
    if(remote&&hasMeaningful(remote)){
      const rc=countData(remote);
      if(lc.sessions<rc.sessions||lc.catches<rc.catches){
        const ok=confirm('VÉDELEM: a GitHub mentés több adatot tartalmaz, mint a telefonon lévő adat.\n\nTelefon: '+lc.sessions+' túra, '+lc.catches+' fogás\nGitHub: '+rc.sessions+' túra, '+rc.catches+' fogás\n\nBiztosan felülírod a GitHub latest mentést ezzel a kevesebb adattal?');
        if(!ok)throw new Error('Mentés megszakítva: a GitHubon több adat van.');
      }
    }
  }
  async function sync(auto){if(running){toast('Már fut egy GitHub mentés. Várd meg, amíg befejeződik.');log('Második mentés blokkolva: az előző még fut.');return;}running=true;try{const c=cfg();check(c);if(typeof githubClearLog==='function')githubClearLog();log('Stabil GitHub mentés indul.');const local=typeof getDB==='function'?getDB():{};await guard(c,local);const pack=await sanitize(local,c);for(const im of pack.imgs){await put(c,im.path,im.base64,'KapásPont kép mentés')}const backup={schemaVersion:4,backupType:'github-sync-full',app:'KapásPont',appVersion:'v29.4',release:'overwrite-guard-active-button',exported:new Date().toISOString(),storage:storage(pack.db,pack.imgs.length),fishImageAssets:[],...pack.db};const js=JSON.stringify(backup,null,2);if(js.length<100)throw new Error('Túl kicsi JSON mentés, nem írom felül a backupot.');const ts=stamp();await put(c,fpath(c,'backups/full_backup_'+ts+'.json'),enc(js),'KapásPont időbélyeges backup','create');for(const s of (pack.db.sessions||[])){const p=fpath(c,'sessions/'+(s.date||'datum-nelkul')+'_'+slug(s.location||'hely')+'_'+slug(s.id)+'_'+ts+'.json');await put(c,p,enc(JSON.stringify({schemaVersion:2,backupType:'session',app:'KapásPont',exported:new Date().toISOString(),session:s},null,2)),'KapásPont horgászat mentés','create')}await put(c,fpath(c,'latest/full_backup_latest.json'),enc(js),'KapásPont latest backup frissítés');const man={schemaVersion:2,created:new Date().toISOString(),app:'KapásPont',sessionCount:(pack.db.sessions||[]).length,imageCount:pack.imgs.length,latest:fpath(c,'latest/full_backup_latest.json'),backup:fpath(c,'backups/full_backup_'+ts+'.json')};await put(c,fpath(c,'manifest.json'),enc(JSON.stringify(man,null,2)),'KapásPont manifest frissítés');toast('Mentés kész: '+new Date().toLocaleString('hu-HU'));log('Mentés kész. Üres / kevesebb adat felülírása elleni védelemmel.');if(typeof renderStorageOverview==='function')renderStorageOverview();}catch(e){console.error(e);toast('Mentési hiba: '+e.message);log('Mentési hiba: '+e.message)}finally{running=false;}}
  function activateButtons(){document.querySelectorAll('button').forEach(b=>{const t=(b.textContent||'').toLowerCase();if(t.includes('mentés githubra most')){b.disabled=false;b.classList.remove('disabled');b.style.pointerEvents='auto';b.onclick=function(ev){ev.preventDefault();ev.stopPropagation();sync(false);};}})}
  function install(){if(typeof getDB!=='function'){setTimeout(install,250);return}window.githubScheduleAutoBackup=function(){log('Automatikus GitHub háttérmentés kihagyva. Kézi mentés használandó.');};window.githubSyncNow=function(auto){return sync(false)};try{const c=cfg();c.auto=false;localStorage.setItem('kapaspont_github_sync',JSON.stringify(c));}catch(e){}activateButtons();setInterval(activateButtons,1000);log('Stabil GitHub mentés aktív: gomb aktiválva, üres/kevesebb adat védelemmel.');}
  install();
})();